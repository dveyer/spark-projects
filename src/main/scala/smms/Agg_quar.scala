...
class Agg_quar(propMap: Map[String, String], sparkMap: Map[String, String], pdate: String, tools: Tools) extends SparkBase(propMap, sparkMap) {
  ...
  def RunJob():Boolean= {
    ...
    if (spark != null)
      try {
        if (tools.checkExistFolder(smms_agg_rating_quar_path)){
          tools.removeFolder(smms_agg_rating_quar_path)
        }
        ...
        if (Integer.parseInt(P_DATE.substring(5, 7))%3==0 && P_DATE.substring(8,10) == "01") {
          val df = d.filter(
            $"business_date".cast(org.apache.spark.sql.types.DateType) >= tools.addMonths(P_DATE,-3) &&
              $"business_date".cast(org.apache.spark.sql.types.DateType) < P_DATE)
            .join(
          ...
          df.join(sl, trim(df("id")) === trim(sl("id"))).select(
            ...
            .save(smms_agg_rating_quar_path)
        }
        else {
          val df = d.filter(
            $"business_date".cast(org.apache.spark.sql.types.DateType) >= tools.firstMonthDay(P_DATE,"yyyy-MM-dd") &&
              $"business_date".cast(org.apache.spark.sql.types.DateType) <= P_DATE)
            .join(
              sl, trim(d("id")) === trim(sl("id"))).select(
            current_date().as("business_date"),
